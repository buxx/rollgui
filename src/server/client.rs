...
impl Client {
    ...
    fn check_response(&self, response: Response) -> Result<Response, ClientError> {
        ...
        if response.status().is_client_error() {
            return Err(ClientError::ClientSideError {response: response.text().unwrap()})
        }

        if !response.status().is_success() {
            return Err(ClientError::ServerSideError {response: response.text().unwrap()})
        }
    ...
    pub fn create_player(&self, name: &str) -> Result<Player, ClientError> {
        ...
        data.insert("background_story".to_string(), Value::String("".to_string()));
        data.insert("max_life_comp".to_string(), Value::Number(Number::from(0u64)));
        data.insert("hunting_and_collecting_comp".to_string(), Value::Number(Number::from(0u64)));
        data.insert("find_water_comp".to_string(), Value::Number(Number::from(0u64)));
    ...
    pub fn get_tiles_data(&self) -> Result<Value, ClientError> {
        ...
        let response: Response = self.check_response(
            self.client.get(url.as_str()).send().unwrap()
        )?;
    ...
    pub fn get_zone_data(&self, world_row_i: i32, world_col_i: i32) -> Result<Value, ClientError> {
        ...
        let url = format!("{}/zones/{}/{}", self.get_base_path(), world_row_i, world_col_i);
        let response: Response = self.check_response(
            self.client.get(url.as_str()).send().unwrap()
        )?;
    ...
    pub fn get_zone_characters(&self, world_row_i: i32, world_col_i: i32) -> Result<Vec<Character>, ClientError> {
        println!("Retrieve zone from server");
        let url = format!("{}/zones/{}/{}/characters", self.get_base_path(), world_row_i, world_col_i);
        let response: Response = self.check_response(
            self.client.get(url.as_str()).send().unwrap()
        )?;
