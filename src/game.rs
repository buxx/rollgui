...
impl Game for MyGame {
    ...
    fn interact(&mut self, input: &mut MyGameInput, window: &mut Window) {
        if let Some(pending_illustration) = self.pending_illustration.clone() {
            ...
            // FIXME BS NOW: log error (and sentry) if not found
            self.server.as_ref().unwrap().client.cache_media(&pending_illustration).unwrap();
            self.illustration = Some(graphics::Image::new(window.gpu(), format!("cache/{}", &pending_illustration)).unwrap());
        }
