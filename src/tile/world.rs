...
const APPEARANCES: [(&str, TileAppearance); 6] = [
    ("JUNGLE", TileAppearance{
        back: color::BLACK,
        fore: color::GREEN,
        ascii: Some(gui::CHAR_TROPICAL_TREE),
    }),
    ("PLAIN", TileAppearance{
        back: color::BLACK,
...
impl Tiles {
    pub fn new(legend: &str) -> Result<Self, RollingError> {
        let default_appearances: HashMap<&str, TileAppearance> = APPEARANCES.iter().cloned().collect();
        ...
        for line in legend.lines() {
            ...
            if let Some(new_appearance) = default_appearances.get(id) {appearance = *new_appearance}
